...
-- Helper function & trigger (run outside transaction)
...
BEGIN
  INSERT INTO public.users(
    id, email, first_name, last_name, date_of_birth, role, ministry_id, created_at
  )
  VALUES (
    NEW.id,
    NEW.email,
...
-- All the rest in one transaction
BEGIN;
...
DROP POLICY IF EXISTS ministries_update_elevated ON public.ministries;
DROP POLICY IF EXISTS ministries_delete_elevated ON public.ministries;
